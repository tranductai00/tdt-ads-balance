...
async function bridge(action, payload = {}) {
  ...
  return fetchJsonWithRetry(`${serverBase}/outlookBridge`, {
    method: "POST",
...
async function getAdsCheckTab({ autoOpen = false } = {}) {
  const tabs = await chrome.tabs.query({ url: "https://adscheckv6.smit.vn/app/adscheck-pro*" });
...
async function syncScrapedResult(scraped, reason = "manual") {
  ...
  const result = await bridge("adsCheckSync", {
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status !== "complete" || !tab?.url?.startsWith(ADSCHECK_URL)) return;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message?.type === "CHECK_STATUS") {
      ...
      const status = device.authorized ? await bridge("adsCheckStatus") : null;
    ...
    if (message?.type === "SAVE_SETTINGS") {
      ...
      await updateAlarm();
      const cfg = await getSettings();
      if (cfg.paired) await bridge("adsCheckConfigure", { settings: message.settings?.serverSettings || {} });
    ...
    if (message?.type === "OPEN_TBALANCE") {
      const cfg = await getSettings();
      return chrome.tabs.create({ url: `${normalizeServerBase(cfg.serverBase)}/adscheck` });
    }
